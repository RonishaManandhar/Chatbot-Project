...
class AuthenticationTests(BaseTestCase):
    ...
    def test_correct_password_logs_verified_customer_in_directly(self):
        response = self.client.post("/login", data={
            "email": self.customer.email,
        ...
        with self.client.session_transaction() as session:
            self.assertEqual(str(self.customer.id), session.get("_user_id"))
            self.assertNotIn("pending_login_user_id", session)
            self.assertNotIn("pending_login_remember", session)
    ...
    def test_login_preserves_safe_next_destination(self):
        response = self.client.post(
            "/login?next=/customer/my-profile",
            data={
                "email": self.customer.email,
                "password": "Customer123!",
            },
        ...
        self.assertEqual(302, response.status_code)
        self.assertTrue(
            response.headers["Location"].endswith("/customer/my-profile")
        )
